...
def tag_sorter(tag_list):
    '''shorter tags with the same appris class tend to be better, so sort the list of tags by length to take the shortest'''
...
'''this section is for exon skipping functions that rely on a slightly cleaned up reference file'''
...
'''here the effect file is leafcutter junction annotation output'''
effectDf = pd.read_csv(effectFile,sep='\t')

'''for ONT data, the 'end' column needs to be moved by 1 to match the reference files. This is not the case for 10x data'''
if dataType == 'ONT':
    effectDf['end'] = effectDf['end'] + 1

'''import the junction reference file only keep the protein coding rows'''
...
'''run the main junction annotation script'''
